...
function BudgetMateDashboard() {
  ...
  const getTransactions = async () => {
    try {
      const response = await axios.get(`${apiUrl}/get-transactions`);

      if (response.data.transactions) {
        setTransactions(
          response.data.transactions.map((t) => ({ ...t, amount: parseFloat(t.amount) }))
        );
      }
    } catch (error) {
  ...
  const deleteTransaction = async (transactionId) => {
    try {
      const { data } = await axios.post(`${apiUrl}/delete-transaction`, {
  ...
  const handleTransactionUpdate = async (id, newAmount, newDescription) => {
    try {
      const { data } = await axios.post(`${apiUrl}/update-transaction`, {
        transaction_id: id,
        amount: newAmount,
      ...
      if (data.success) {
        setTransactions((prev) =>
          prev.map((t) =>
            t.id === id ? { ...t, amount: parseFloat(newAmount), description: newDescription } : t
          )
        );
  ...
  const totalExpenses = transactions.reduce(
    (acc, t) => (t.type === "expense" ? acc + t.amount : acc), 0
  );
  const totalIncome = transactions.reduce(
    (acc, t) => (t.type === "income" ? acc + t.amount : acc), 0
  );
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-purple-400 to-pink-400 p-4 md:p-6 lg:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          <h1 className="text-3xl font-bold text-green-800">💸 BudgetMate Dashboard</h1>
          <button className="bg-teal-300 hover:bg-teal-600 text-white px-4 py-2 rounded-lg" onClick={handleLogout}>
            Logout
          </button>
        </div>
...
<div className="bg-white shadow-lg rounded-lg p-6 border-2 border-teal-700">
  <h2 className="text-xl font-semibold text-teal-700 mb-2">🎯 Monthly Budget</h2>
  {!isEditingBudget ? (
    <>
      {/* Original Budget */}
      ...
      {/* Total Budget with Income */}
      <p className="text-md text-gray-700">Budget + Total Income:</p>
      <p className="text-3xl font-bold text-green-700 mb-2">
        ₱{monthlyBudget !== null ? (monthlyBudget + totalIncome).toFixed(2) : '--'}
      </p>
  ...
  {monthlyBudget !== null && totalExpenses > monthlyBudget + totalIncome && (
...
{/* Balance */}
<div className="bg-white shadow-lg rounded-lg p-6 border-2 border-teal-700">
  <h2 className="text-xl font-semibold text-teal-700 mb-2">💰 Balance</h2>
  <p className="text-3xl font-bold text-teal-600">₱{balance.toFixed(2)}</p>
</div>

{/* Total Expenses */}
<div className="bg-white shadow-lg rounded-lg p-6 border-2 border-teal-700">
  <h2 className="text-xl font-semibold text-teal-700 mb-2">💸 Total Expenses</h2>
  <p className="text-3xl font-bold text-red-600">₱{totalExpenses.toFixed(2)}</p>
</div>

{/* Total Income */}
<div className="bg-white shadow-lg rounded-lg p-6 border-2 border-teal-700">
  <h2 className="text-xl font-semibold text-teal-700 mb-2">💵 Total Income</h2>
...
<div className="bg-white shadow-lg rounded-lg p-6 border-2 border-teal-700 mb-6">
  <h2 className="text-xl font-semibold text-teal-700 mb-4">📂 Monthly Expenses</h2>
  {categories.length > 0 ? (
    <ul className="space-y-2">
      {categories.map(([category, total]) => {
        const isTooHigh = total > 1000;
        const notNeeds = ["entertainment", "games", "shoes", "clothing", "luxury", "gadget", "vacation"];
        const isNotNeed = notNeeds.some(keyword =>
          category.toLowerCase().includes(keyword)
        );

        return (
          <li key={category} className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-2 border-b pb-2">
            <div>
              <span className="font-medium">{category}</span>
              {isTooHigh && (
                <span className="ml-2 text-red-500 text-sm font-semibold">Too High 💥</span>
              )}
              {isNotNeed && (
                <span className="ml-2 text-yellow-500 text-sm font-semibold">Not a Need ⚠️</span>
              )}
            </div>
          ...
          <div className="max-h-72 overflow-y-auto">
            {transactions.length > 0 ? (
              transactions.map((transaction) => (
                <div key={transaction.id} className="flex justify-between items-center mb-3">
                    ...
                    <div className="flex-1 flex flex-col sm:flex-row gap-2 items-center">
                      ...
                      <div className="flex gap-2 mt-2 sm:mt-0">
                        <button
                          ...
                          onClick={() => handleTransactionUpdate(transaction.id, editAmount, editDescription)}
                        >
                          Save
                        </button>
                        <button
