...
function AddTransactionModal({ hide, refreshTransactions, userId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Send the data to the server to add the transaction
      const response = await axios.post(`${apiUrl}/add-transaction`, {
        userId, // Pass the userId to associate the transaction with the correct user
        description,
      ...
      if (response.data.success) {
        // Refresh the transactions in the parent component
        refreshTransactions();
